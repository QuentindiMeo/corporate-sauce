...
test.describe('Feature: gallery view toggle (theme ⇄ date)', () => {
	test('Given a first visit, When the page loads, Then the gallery opens by category', async ({
		page,
	}) => {
		await page.goto('/');
	...
	test('Given the category view, When the toggle is clicked, Then the date flow replaces it', async ({
		page,
	}) => {
		await page.goto('/');
	...
	test('Given the toggle, When the view switches, Then its icon and label follow the active view', async ({
		page,
	}) => {
		await page.goto('/');
		const toggle = page.locator('[data-view-toggle]');
	...
	test('Given the date view, When the toggle is clicked again, Then the category view comes back', async ({
		page,
	}) => {
		await page.goto('/');
		const toggle = page.locator('[data-view-toggle]');
	...
	test('Given a chosen view, When the page is reloaded, Then the choice persists', async ({
		page,
	}) => {
		await page.goto('/');
	...
	test('Given the toggle, When the view switches, Then the change is announced politely', async ({
		page,
	}) => {
		await page.goto('/');
	...
	test('Given the date view, When a card is clicked, Then the modal still opens', async ({
		page,
	}) => {
		await page.goto('/');
		await page.locator('[data-view-toggle]').click();
...
test.describe('Feature: month rail (date view)', () => {
	...
	test('Given the category view, When the page loads, Then only the category rail is shown', async ({
		page,
	}) => {
		await page.goto('/');
	...
	test('Given the date view, When it is active, Then the rail switches to months', async ({
		page,
	}) => {
		await page.goto('/');
		await page.locator('[data-view-toggle]').click();
	...
	test('Given the month rail, When the page is at the top, Then the first month is current', async ({
		page,
	}) => {
		await page.goto('/');
		await page.locator('[data-view-toggle]').click();
	...
	test('Given a month rail link, When it is clicked, Then it leads to the matching month section', async ({
		page,
	}) => {
		await page.goto('/');
		await page.locator('[data-view-toggle]').click();

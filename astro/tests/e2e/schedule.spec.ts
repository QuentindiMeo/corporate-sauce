...
test.describe("Feature: scheduled state follows the browser clock", () => {
  ...
  test("Given a far-past clock, When the page loads, Then every post is scheduled and counted", async ({ page }) => {
  ...
  test("Given the modal is opened, Then its mention follows the browser clock too", async ({ page }) => {
    await page.clock.setFixedTime(AVANT);
    await page.goto("/");
...
test.describe("Feature: scheduled state without JavaScript", () => {
  ...
  test("Given JS is disabled, When the page loads, Then CSS alone shows exactly one state per card", async ({
    ...
    const visibles = await page.locator('[data-view-panel="theme"] .post-card__scheduled:visible').count();

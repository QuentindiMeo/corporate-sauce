...
export function aThemeRowVm(
	category: Category,
	posts: readonly Post<ImageMetadata>[],
): ThemeRowViewModel {
	return toThemeRowViewModels([{ category, posts }])[0];
}

export function aMonthRowVm(
	monthKey: string,
	posts: readonly Post<ImageMetadata>[],
): MonthRowViewModel {
	return toMonthRowViewModels([
		{ monthKey, month: new Date(`${monthKey}-01T00:00:00.000Z`), posts },
	])[0];
}

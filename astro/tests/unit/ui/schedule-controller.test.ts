...
const gabarit = (id: string) => document.querySelector(`template[data-post-template="${id}"]`) as HTMLTemplateElement;
...
describe("Feature: schedule controller — cards", () => {
  ...
  it("Given a card, When its state changes, Then its accessible name follows", () => {
    ...
    expect(lien("carte-a-venir").getAttribute("aria-label")).toBe("Ouvrir le carrousel « À venir » (à venir)");
  });
});
...
describe("Feature: schedule controller — modal templates", () => {
  it("Given inert template content, When the schedule is applied, Then it is corrected too", () => {
    ...
    const dedans = (id: string) => gabarit(id).content.querySelector("[data-published-at]") as HTMLElement;

...
export function toPostViewModel(post: Post<ImageMetadata>): PostViewModel {
  ...
  const aria = {
    ...
    description: carousel ? `Contient ${pages.length} page${pages.length > 1 ? "s" : ""}.` : undefined,
  };
...
export function toThemeRowViewModels(rows: readonly ThemeRow<ImageMetadata>[]): ThemeRowViewModel[] {
  return rows.map((row) => ({
...
export function toMonthRowViewModels(rows: readonly MonthRow<ImageMetadata>[]): MonthRowViewModel[] {
  return rows.map((row) => ({
